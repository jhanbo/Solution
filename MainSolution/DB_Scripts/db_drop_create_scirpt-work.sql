...
CREATE  TABLE IF NOT EXISTS `itg_wd`.`correspondence` (
  ...
  INDEX `correspondence_state_fk` (`state` ASC) ,
  CONSTRAINT `correspondence_state_fk`
    FOREIGN KEY (`state` )
    REFERENCES `itg_wd`.`correspondence_states` (`idcorrespondence_states` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE  TABLE IF NOT EXISTS `itg_wd`.`assignment` (
  ...
  INDEX `assignment_itg_user_fk` (`correspondence_idcorrespondence` ASC) ,
